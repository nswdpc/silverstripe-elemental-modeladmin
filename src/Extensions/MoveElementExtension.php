...
class MoveElementExtension extends Extension
{
    ...
    public function getElementalAreaRelations(): array
    {
        ...
        $classes = $this->getOwner()->config()->get('supported_parent_classes');
        ...
        $suffix = function (string &$value): void {
            $value .= "ID";
        };
    ...
    public function getApplicableElementalAreas(bool $exclude_current = true)
    {
        ...
        $classes = $this->getOwner()->getAreaOwnerClasses();
        ...
        if ($area_ids !== []) {
            ...
            if ($exclude_current) {
                ...
                $list = $list->exclude([ 'ID' => $this->getOwner()->ParentID ]);
            }
